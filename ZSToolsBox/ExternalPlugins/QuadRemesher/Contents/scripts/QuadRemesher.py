...
class QuadRemesherWindow(QWidget):
	...
	def initUI(self):
		...
		# -- tooltips: --
		curvatureAdaptivness_Tooltip = "可用于控制四边形的大小如何局部适应曲率。\n该值越高，高曲率区域的四边形就越小。\n将其设为 0，即可获得大小均匀的四边形。"
		adaptQuadCount_Tooltip = "自适应四边形数量：\n勾选：生成比指定数量更多的多边形，以适配高曲率区域。\n不勾选（默认）：更严格地遵循目标四边形数量。\n建议保持不勾选状态，以便更好地遵循目标四边形数量。更多说明请参阅文档。"
		useVertexColors_Tooltip = "使用“顶点颜色”控制四边形的尺寸与密度。"
		vertexColorWidget_Tooltip = "定义用于绘制的颜色，以便在局部控制所需的四边形密度变化（在RGB模式下使用“绘制顶点颜色工具”）\n. 从0.25开始 →“将密度除以4” = 大四边形 = 青色\n. 到4结束 →“将密度乘以4” = 小四边形 = 红色。"
		useMaterials_Tooltip="若启用此选项，QuadRemesher 将利用现有的“材质”在材质边界上引导四边形重网格化操作。\n重网格化后，材质 ID 将得以保留。"
		useHardEdges_Tooltip="若开启此功能，四边面重网格化工具将利用现有的“硬化/软化边”来引导四边面重网格化过程。\n由于布尔运算会自动设置硬边/软边，建议在对布尔运算结果进行重网格化时启用该功能。"
		useNormalsCreasing="若启用，QuadRemesher 将使用现有的“法线”，在法线形成折痕的边环上引导重新网格化。\n由于大多数操作会自动设置法线，建议启用此功能。\n对于平滑的有机形态，建议取消勾选。"
		detectHardEdges_Tooltip="若启用此选项，QuadRemesher 将根据几何体自动检测并计算硬边（综合运用边的角度及其他几何参数）。\n如果你尚未定义部分硬边/软边，且希望 QuadRemesher 自动识别硬角度，该功能会非常实用。\n如果勾选了“使用现有硬边/软边”，建议取消勾选“检测硬边”。\n对于平滑的有机形态模型，建议取消此选项。"
		symToolTip = "这些选项可用于执行对称四边形重新网格化操作。可同时组合使用全部3个对称轴。\n注意：这些轴为局部坐标系轴！建议将操控器设为“对象”模式，以便更清晰地查看局部坐标系轴。"
		remeshButton_Tooltip = "对选中的网格执行四边形重网格化操作。"
		...
		self.targetQuadCountSpinBox.setToolTip("设置所需的四边形数量")
		...
		try:
			...
			import traceback
			sys.stdout.write("添加新闻按钮时出错：\n" + str(traceback.format_exc())+"\n")
		
		# --- "Remesh It" button ---
		self.remeshButton = QPushButton()
		self.remeshButton.setText('<<   开 始 拓 扑   >>')
	...
	def vertexColorWidget_valueChanged(self): # -------------------- ColorSlider Changed ---------------
		try:
			...
			sys.stdout.write("异常：在 vertexColorWidget_valueChanged 中……\n")
	...
	def remeshButton_onClicked(self): # ---------------------------------------------- Do REMESH FUNC -----------------------
		try:
			...
			if (fbxWasLoaded == 0) :
				print("正在为 QuadRemesher 加载FBX插件...\n")
			...
			if (inputSelection==None) or (len(inputSelection)!=1):
				cmds.confirmDialog( title='QuadRemesher 警告', message='您必须选择**一个且仅一个**网格对象！', button=['OK'])
				return
			...
			if ProgressValueFloat < 0:   # RemeshingFailed!
				...
				return
			if Aborted:
				if (verboseDebug): sys.stdout.write("用户已中止网格重构操作\n")
				return
			...
			if (verboseDebug): sys.stdout.write("正在导入拓扑重构工具…\n")
			if True:
				...
				if (verboseDebug): sys.stdout.write("选择重新拓扑……\n")
				try:
					...
					sys.stdout.write('异常：正在查找导入的节点…')

				if (verboseDebug): sys.stdout.write("复制属性……\n")
				try:
					...
					import traceback
					sys.stdout.write("复制属性时出现异常……：" + str(traceback.format_exc())+"\n")
					
				if (verboseDebug): sys.stdout.write("隐藏输入网格……\n")
				try:
					# Hide SourceNode			
			...
			import traceback
			sys.stdout.write("异常：在 remeshButton_onClicked 中……\n")
	...
	def licenseManagerButton_onClicked(self): # ------------------------------------ LicenseManager ----------------------------
		try:
			...
			import traceback
			sys.stdout.write("异常：在 licenseManagerButton_onClicked 方法中……\n")
			sys.stdout.write(str(traceback.format_exc()) + "\n")
	...
	def webDocButton_onClicked(self): # ------------------------------------ WebDoc buton click ----------------------------
		try:
			...
			import traceback
			sys.stdout.write("异常：执行 webDocButton_onClicked 时发生……\n")
			sys.stdout.write(str(traceback.format_exc()) + "\n")
	...
	def newVerButton_onClicked(self): # ---------------------------------------------- News button click -------------------
		try:
			...
			import traceback
			sys.stdout.write("异常：在 newVerButton_onClicked 中……\n")
			sys.stdout.write(str(traceback.format_exc()) + "\n")
