...
#	asFace.recompile_modular_rig()
	...
	ModularRigController.add_module (module_name='asFace', class_= module_class , parent_module_name='Neck')
...
#			hierarchy_controller.add_socket('foot'+side+'_socket', unreal.RigElementKey(type=unreal.RigElementType.BONE, name='foot'+side), unreal.Transform(location=[0,0,0],rotation=[-0,0,-0],scale=[1,1,1]), False, unreal.LinearColor(1, 1, 1, 1), '')
		...
		ModularRigController.add_module (module_name='Spine', class_= module_class , parent_module_name='Root')
...
#ModularRigController.bind_module_variable('Root:Spine','Module Settings','test3')
		...
		ModularRigController.add_module (module_name='Neck', class_= module_class , parent_module_name='Spine')

		for side in ['_r','_l']:
			module_class = unreal.load_class(None, '/ControlRigModules/Modules/Shoulder.Shoulder_C')
			ModularRigController.add_module (module_name='Shoulder'+side, class_= module_class , parent_module_name='Spine')
			module_class = unreal.load_class(None, '/ControlRigModules/Modules/Arm.Arm_C')
			ModularRigController.add_module (module_name='Arm'+side, class_= module_class , parent_module_name='Shoulder'+side)
			module_class = unreal.load_class(None, '/ControlRigModules/Modules/Leg.Leg_C')
			ModularRigController.add_module (module_name='Leg'+side, class_= module_class , parent_module_name='Root')
			module_class = unreal.load_class(None, '/ControlRigModules/Modules/Foot.Foot_C')
			ModularRigController.add_module (module_name='Foot'+side, class_= module_class , parent_module_name='Leg'+side)

			for finger in ['index','middle','ring','pinky','thumb']:
				module_class = unreal.load_class(None, '/ControlRigModules/Modules/Finger.Finger_C')
				ModularRigController.add_module (module_name='Finger'+finger+side, class_= module_class , parent_module_name='Arm'+side)
	...
	asConnect ('root' , 'Root')
	asConnect ('Root/spine_01_socket' , 'Spine/Spine Primary')
	asConnect ('spine_05' , 'Spine/Spine End Bone')
	asConnect ('neck_socket' , 'Neck/Neck Primary')
	asConnect ('neck_01' , 'Neck/Neck Start Bone')
	asConnect ('head' , 'Neck/Head Bone')

	for side in ['_r','_l']:
		asConnect ('shoulder'+side+'_socket' , 'Shoulder'+side+'/Shoulder Primary')
		asConnect ('spine_05' , 'Shoulder'+side+'/Chest Bone')
		asConnect ('hand'+side+'_socket' , 'Arm'+side+'/Arm Primary')
		asConnect ('spine_01_socket' , 'Leg'+side+'/Leg Primary')
		asConnect ('thigh'+side , 'Leg'+side+'/Thigh Bone')
		asConnect ('foot'+side , 'Leg'+side+'/Foot Bone')
		#Knee Bone Connector added in Unreal 5.5
		asConnect ('calf'+side , 'Leg'+side+'/Knee Bone')
		asConnect ('foot'+side+'_socket' , 'Foot'+side+'/Foot Primary')
		asConnect ('ball'+side , 'Foot'+side+'/Ball Bone')
		asConnect ('foot'+side , 'Foot'+side+'/Foot Bone')

		for finger in ['index','middle','ring','pinky','thumb']:
			asConnect ('hand'+side+'_socket' , 'Finger'+finger+side+'/Finger Primary')
			if finger == 'thumb':
				asConnect (finger+'_01'+side , 'Finger'+finger+side+'/Start Bone')
			else:
				asConnect (finger+'_metacarpal'+side , 'Finger'+finger+side+'/Start Bone')
			asConnect (finger+'_03'+side , 'Finger'+finger+side+'/End Bone')

	if includeFaceRig:
		asConnect ('head' , 'asFace/Root')
